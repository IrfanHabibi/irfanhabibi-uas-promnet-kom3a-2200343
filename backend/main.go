...
func main() {
	...
	router.HandleFunc("/inventory_irfanhabibi", GetInventoryItems).Methods("GET")
	router.HandleFunc("/inventory_irfanhabibi", CreateInventoryItem).Methods("POST")
	router.HandleFunc("/inventory_irfanhabibi/{id}", GetInventoryItem).Methods("GET")
	router.HandleFunc("/inventory_irfanhabibi/{id}", UpdateInventoryItem).Methods("PUT")
	router.HandleFunc("/inventory_irfanhabibi/{id}", DeleteInventoryItem).Methods("DELETE")
	router.HandleFunc("/inventory_irfanhabibi/search", SearchInventoryByName).Methods("GET")
...
func GetInventoryItems(w http.ResponseWriter, r *http.Request) {
	...
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_irfanhabibi")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := DB.Prepare("INSERT INTO inventory_irfanhabibi(nama_barang, jumlah, harga_satuan, lokasi, deskripsi) VALUES(?,?,?,?,?)")
	if err != nil {
		panic(err.Error())
	}
...
func GetInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_irfanhabibi WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := DB.Prepare("UPDATE inventory_irfanhabibi SET nama_barang = ?," +
...
func DeleteInventoryItem(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := DB.Prepare("DELETE FROM inventory_irfanhabibi WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func SearchInventoryByName(w http.ResponseWriter, r *http.Request) {
	...
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_irfanhabibi WHERE nama_barang LIKE ?", "%"+searchTerm+"%")
	if err != nil {
...
func InitDB() {
	DB, err = sql.Open("mysql", "root:@/db_2200343_irfanhabibi")
	if err != nil {
		panic(err)
	}
